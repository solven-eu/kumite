...
// Adjusted from `WebSocketMessageBrokerSecurityConfiguration` for reactive.
@Order(Ordered.HIGHEST_PRECEDENCE + 100)
@Import(MessageMatcherAuthorizationManagerConfiguration.class)
final class ReactiveWebSocketMessageBrokerSecurityConfiguration
	...
	private static final AuthorizationManager<Message<?>> ANY_MESSAGE_AUTHENTICATED =
			MessageMatcherDelegatingAuthorizationManager.builder().anyMessage().authenticated().build();
	...
	ReactiveWebSocketMessageBrokerSecurityConfiguration(ApplicationContext context) {
	...
	public void configureClientInboundChannel(ChannelRegistration registration) {
		ChannelInterceptor csrfChannelInterceptor =
				getBeanOrNull(CSRF_CHANNEL_INTERCEPTOR_BEAN_NAME, ChannelInterceptor.class);
	...
	public void afterSingletonsInstantiated() {
		SimpleUrlHandlerMapping mapping =
				getBeanOrNull(SIMPLE_URL_HANDLER_MAPPING_BEAN_NAME, SimpleUrlHandlerMapping.class);
		if (mapping == null) {
			return;
		}
	...
	private void configureCsrf(SimpleUrlHandlerMapping mapping) {
		Map<PathPattern, Object> mappings = mapping.getHandlerMap();
		for (Object object : mappings.values()) {
			if (object instanceof SockJsHttpRequestHandler) {
				...
				throw new IllegalStateException("Bean " + SIMPLE_URL_HANDLER_MAPPING_BEAN_NAME
						+ " is expected to contain mappings to either a "
						+ "SockJsHttpRequestHandler or a WebSocketHttpRequestHandler but got "
						+ object);
			}
		}
	}
