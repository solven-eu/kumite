...
import SnakeTools from "./kumite-snake-tools.js";
...
export default {
	...
	setup(props) {
		...
		function renderMove(move) {
			...
			const group = SnakeTools.renderMove(renderer, move);
		...
		onMounted(() => {
			...
			renderer.setSize(SnakeTools.width(), SnakeTools.height());

			console.debug("Rendering board", board);
			{
				const positions = board.positions;

				SnakeTools.renderSupport(renderer);

				SnakeTools.renderPositions(renderer, positions);
			}
			renderer.render();
